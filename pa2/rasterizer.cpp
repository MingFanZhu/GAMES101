...
{   
    ...
    Vector3f vp0 = point - _v[0];
    Vector3f vp1 = point - _v[1];
    Vector3f vp2 = point - _v[2];
    float c1 = v0.cross(vp0).z();
    float c2 = v1.cross(vp1).z();
    float c3 = v2.cross(vp2).z();
...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
    ...
    for(int col = x_min_int; col <= x_max_int; col++){
        for(int row = y_min_int; row <= y_max_int; row++){
            if(insideTriangle(col, row, t.v)){
                ...
                if(depth_buf[index] > z_interpolated){
                    ...
                    set_pixel({(float)col, (float)row, 1}, t.getColor());
                }
